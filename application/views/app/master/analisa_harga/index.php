...
<div id="dialog-form" class="easyui-dialog" style="width:auto; height:auto; padding: 10px 20px" 
    ...
    <table>
	<tr>
            <td style="width: 45%">Periode (Tahun Semester)</td>
			<td>:</td>
            <td><input name="id_periode" class="easyui-validatebox" required="true" size="13" maxlength="10"></td>
	</tr>
	<tr>
            <td>Kode</td>
			<td>:</td>
...
<script>
    ...
    $('#datagrid').datagrid({ 
		...
		url: '<?php echo site_url('app/analisa_harga'); ?>/'+jenis+'?analisa_harga=true', nowrap:false,
		...
		columns:[[
			{field:'kode',title:'Kode',width:80,
				formatter:function(value,row,index){
					var strReturn = value;
					strReturn += 
					'<div style="float:right;"><a href="javascript:void(0)" class="icon-add2" onclick="addDetail('+row.id+');" title="Add Komponen"></a>'+
					'&nbsp;&nbsp;&nbsp;'+
					'<a href="javascript:void(0)" class="icon-edit2" onclick="edit('+index+');" title="Edit"></a>'+
					'&nbsp;&nbsp;&nbsp;'+
					'<a href="javascript:void(0)" class="icon-trash" onclick="del('+index+')" title="Delete"></a></div>';
					return strReturn;
				}
			},
