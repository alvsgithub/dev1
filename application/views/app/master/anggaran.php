...
<div id="dlg-import" class="easyui-dialog" style="width:auto; height:auto; padding: 10px 20px" 
    ...
    <form id="form_import" method="post" action="<?php echo site_url('app/anggaran/run_import') ?>" enctype="multipart/form-data" role="form">
        ...
        <input type="file" id="import_browse" name="anggaran" class="easyui-linkbutton" onChange="document.getElementById('periode').value = document.getElementById('id_periode').value;this.form.submit();">
    </form>
</div>
<section_easyui>  
...
<div id="dlg" class="easyui-dialog" style="width:auto; height:auto; padding: 10px 10px;" 
	...
	<tr>
        ...
        <tr>
            ...
            <td>:</td>
            <td><input name="harga_pagu" class="easyui-numberbox" min="0" precision="2" decimalSeparator="," groupSeparator="." required="true" style="width: 40%;text-align:right;"/></td>
	</tr>
    </table>
    </form>
    <div id="dbtn"> 
...
<script type="text/javascript">
    ...
    $(function (){
         $('#datagrid').datagrid({ 
            ...
            url: '<?php echo site_url('app/anggaran/index'); ?>/?item='+jenis+'&periode='+p, nowrap:false,
        ...
        $('#id_periode').combobox({
            onChange:function(newValue,oldValue){
                $('#datagrid').datagrid({
                    url: '<?php echo site_url('app/anggaran/index'); ?>/?item='+jenis+'&periode='+newValue
                });
            }
        });
    ...
    function add(){
        ...
        url = '<?php echo site_url('app/anggaran/create'); ?>';
    }
    ...
    function edit(id){
        ...
        url = '<?php echo site_url('app/anggaran/update'); ?>/'+row.id;
    }
        ...
        if (row){
            $.messager.confirm('Confirm','You are about to delete a record. This cannot be undone. Are you sure?',function(r){
                if (r){
                    ...
                    $.post('<?php echo site_url('app/anggaran/delete'); ?>',{id:row.id},function(result){
                        if (result.success){
